...
class Game {
  ...
  movePlatformToEnd(platform, jumperBounds) {
    if (platform.platform && jumperBounds.x > platform.platform.x + 600) {
  ...
  generateEnemy(){
    ...
    for (let i = 0; i < 4; i++) {
      ...
      let enemy = new __WEBPACK_IMPORTED_MODULE_3__enemy_js__["a" /* default */](this.stage, this.world, x, y);
  ...
  handleNewGameKey(e) {
    ...
    if (e.key === 'Enter') {
        ...
        let game = new __WEBPACK_IMPORTED_MODULE_2__new_game_js__["a" /* default */](this.stage, this.world, this.handleNewGameKey);
        game.startGame();
      }
  }
